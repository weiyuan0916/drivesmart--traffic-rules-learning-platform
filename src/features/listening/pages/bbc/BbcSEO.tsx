...
export function BbcSEODetail({ title, slug, level, durationSeconds, thumbnailUrl }: BbcSEODetailProps) {
  const fullTitle = `${title} — BBC Learning English | DriveSmart`
  const description = `Học bài "${title}" từ BBC Learning English${level ? ` (${level})` : ''}. DriveSmart cung cấp không gian học tập với ghi chú, từ vựng và luyện nghe chép.`
  const canonicalUrl = `${BASE_URL}/listening/bbc/${slug}`
  ...
  const jsonLd = {
    ...
    timeRequired: durationSeconds ? `PT${Math.ceil(durationSeconds / 60)}M` : undefined,
  }
  ...
  const course = {
    ...
    provider: {
      '@type': 'Organization',
      name: 'DriveSmart x BBC Learning English',
      url: BASE_URL,
    },
  }
  ...
  return (
    <Helmet>
      ...
      <meta name="description" content={description} />
      <meta name="keywords" content={`${title}, BBC Learning English, học tiếng Anh, ${level ?? ''}`} />
